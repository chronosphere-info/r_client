...
The 'chronosphere' package includes two demo data sets: a set of ancient topographies (PaleoDEMs, Scotese..) and time series of bio-climatic variables (annual mean temperature and precipitation) from the CHELSA project. ($ref) These datasets can be attached with the data command. 
```{r dat, echo= TRUE}
data(dems)
data(clim)
```
