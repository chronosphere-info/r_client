...
The `chronosphere` package includes two demo data sets: a set of ancient topographies (Scotese and Wright, 2018) and a time series of bio-climatic variables (annual mean temperature and precipitation) from the CHELSA project (Karger et al, 2017a, 2017b). These datasets can be attached with the data command. 
```{r dat, echo= TRUE}
data(dems)
data(clim)
```
...
RasterArrays are fairly easy to construct: one only needs a stack of the data and a regular vector/matrix/array including integers. For instance, the dems object can be recreated from scratch without any problem.
...
Facilitating the accession of items is the primary purpose of RasterArrays. These either focus on the layers (stack items, double bracket operator "[[") or on the elements of the RasterArray (single bracket operator "["). 
...
The above described methods can also be applied to RasterArrays containing more than one variable. However, in this instance, the number of columns automatically defaults to the number of variables in the array. Also, plot titles for each individual plot are not printed. Instead, row labels corresponding to the row names of each variable are added, and when `legend=TRUE`, the default legend title defaults to the variables names in the RasterArray. 

```{r rarray_plot_nvar, echo=TRUE, fig.width=8, fig.height=4.5}
data(clim)

mapplot(clim[1:2,], legend=TRUE)
```

The row labels and legend titles can be edited using the argument `rowlabels` and `legend.title` respectively. Different colours for each variable can also be provided. The number of provided colour palettes should be either 1 or correspond to the number of variables.  
```{r rarray_plot_rowlabs, echo=TRUE, fig.width=8, fig.height=4.5}
data(clim)
